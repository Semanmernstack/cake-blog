...
function Blog({ posts }) {
  ...
  return (
    <div className=" flex flex-col h-screen bg-blue-400 shadow-2xl bottom-1 items-center text-center justify-center max-w-4xl mx-auto md:max-w-5xl lg:max-w-7xl p-2">
      <div className="mt-3">
        ...
        <p className="mt-2 text-left p-2" dangerouslySetInnerHTML={{__html: posts.post.content.html}}></p>
      </div>
      <div className="flex flex-col items-center ">
        <img className="w-14 h-14 rounded-full" src={posts.post.author.avatar.url} alt="avatar" />
        <p className="font-semibold">{posts.post.author.name}</p>
        <p className="mb-3">{posts.post.datePublishes}</p>
      </div>
